...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            # === INSERT STOP‐SIGN LOGIC HERE ===
            if (104.0 <= current_x <= 109.5) and (128.0 <= current_y <= 131.0) and not stop_complete:
                if stop_count < 20:
                    # Force a full brake for 20 frames
                    cmd_throttle = 0.0
                    cmd_brake    = 1.0
                    stop_count  += 1
                else:
                    # After 20 frames, consider the stop “done”
                    stop_complete = True
